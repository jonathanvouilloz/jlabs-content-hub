/**
 * Prépare les réponses et enregistre les mentions employés du lot 2026-08-26.
 *
...
import ws from 'ws';
import { REVIEW_PREPARATIONS } from './reply-reviews-2026-08-26.data.js';
...
if (unexpectedArgs.length > 0) {
  throw new Error(`Arguments non pris en charge : ${unexpectedArgs.join(', ')}`);
}
...
for (const item of REVIEW_PREPARATIONS) {
  ...
  const inWindow = createdAt >= Date.parse('2026-08-16T22:00:00Z') && createdAt < Date.parse('2026-08-30T22:00:00Z');
...
if (!APPLY) {
  const replyCount = REVIEW_PREPARATIONS.filter((item) => item.reply).length;
  const mentionCount = REVIEW_PREPARATIONS.reduce((total, item) => total + item.mentions.length, 0);
  console.log(JSON.stringify({ mode: 'dry-run', reviews: REVIEW_PREPARATIONS.length, replies: replyCount, mentions: mentionCount }));
  await pool.end();
  process.exit(0);
}
...
try {
  ...
  const verification = await client.query<{
    ...
    [project.id, REVIEW_PREPARATIONS.filter((item) => item.reply).map((item) => item.reviewId), ids]
  ).then((result) => result.rows[0]);

  // `public_markers` porte seulement sur les 13 candidats de brouillon : aucun
  // ne doit avoir été marqué envoyé pendant cette préparation DB-only.
  if (verification.rows_with_mentions !== REVIEW_PREPARATIONS.length || verification.mention_items !== 18 || verification.rows_with_expected_draft !== 13 || verification.public_markers !== 0) {
