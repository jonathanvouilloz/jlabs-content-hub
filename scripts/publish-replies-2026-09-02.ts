/**
 * Publication contrôlée du lot Barber Concept préparé le 2026-09-02.
 *
...
// Feu vert explicite du 02.09.2026 : publier seulement les 6 avis dont
// les mentions sont entièrement classifiées. Les 2 avis « Ilias » restent exclus.
const candidates = REVIEW_PREPARATIONS.filter((item) => item.mentions !== null);
if (candidates.length !== 6) {
  throw new Error(`Périmètre autorisé inattendu : ${candidates.length}/6 avis.`);
}
...
for (const candidate of candidates) {
  ...
  if (
    ...
    review.mentioned_employees !== JSON.stringify(candidate.mentions)
  ) {
