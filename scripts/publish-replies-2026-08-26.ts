/**
 * Publication contrôlée du lot Barber Concept préparé le 2026-08-26.
 *
...
const candidates = REVIEW_PREPARATIONS.filter(
  (item): item is typeof item & { reply: string } => typeof item.reply === 'string'
);
...
for (const candidate of candidates) {
  ...
  if (
    ...
    review.mentioned_employees === null
  ) {
  ...
  const result = await pool.query(
    `update seostats.gmb_reviews
     ...
     where project_id = $5 and review_id = $6
       and draft_reply = $1 and replied_at is null and remote_reply_at is null`,
