/**
 * JOB-002 — Chronologie des tentatives d'un job (lecture seule).
 *
...
if (!JOB_ID && !PROJECT) {
	console.error('Usage : --job=<id> ou --project=<slug> [--status=a,b] [--limit=N]');
	process.exit(1);
}
...
async function fetchJobs(): Promise<JobRow[]> {
	...
	const res = await db.execute(sql`
		SELECT j.id, j.type, j.status, j.attempts, j.max_attempts, j.last_error_code, p.slug
	...
	for (const job of jobs) {
		console.log(
			`job ${job.id}  ${job.type}  (${job.slug})  ` +
				`[${job.status}, ${job.attempts}/${job.max_attempts} tentatives` +
				`${job.last_error_code ? `, dernière erreur ${job.last_error_code}` : ''}]`
		);
		...
		for (const a of attempts) {
			...
			console.log(
				`  #${a.attemptNo}  ${a.workerId}  ${hhmmss(a.startedAt)}→${hhmmss(a.finishedAt)}  ` +
					`${OUTCOME_LABEL[a.outcome] ?? a.outcome}${cause}${err}${beats}`
			);
		}
		console.log('');
	}
