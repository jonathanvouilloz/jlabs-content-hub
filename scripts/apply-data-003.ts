/**
 * DATA-003 — Application déterministe du DDL additif (drizzle/manual-data-003.sql).
 *
 ...
 *
 * Lancer : npx tsx scripts/apply-data-003.ts
 * Vérif  : npx tsx scripts/data-001-cartography.ts 2026-07-21  (attendu 35 tables, zéro dérive)
 */
...
async function main() {
	...
	const sqlPath = join(here, '..', 'drizzle', 'manual-data-003.sql');
	...
	console.log('DDL DATA-003 appliqué (idempotent).');

	// Contrôle rapide : les trois tables existent bien.
	const { rows } = await pool.query<{ table_name: string }>(
		`SELECT table_name FROM information_schema.tables
		  WHERE table_schema = 'seostats'
		    AND table_name IN ('monitoring_runs', 'monitoring_steps', 'jobs')
		  ORDER BY table_name`
	);
