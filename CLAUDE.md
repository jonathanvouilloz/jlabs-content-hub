...
>
> ✅ **Le cockpit est en production depuis le 2026-07-27 ~19:40 UTC.** `main` porte `7a8e04b` (fast-forward de `feat/cockpit`, **113 commits**), la prod l'annonce (`/api/whoami` → `version: 7a8e04b`, 9 projets), et les **5 crons** sont déclarés (`tick` horaire, `gmb-publish`, `linkedin-publish`, `gmb-weekly-digest`, `gmb-reviews`). Les 9 cadences hebdo sont **en pause en base** — reprise projet par projet (`scripts/pauses.ts`). L'ancien risque `npm run db:push` (29 tables déclarées pour 61 en base) est **levé** : `main` déclare enfin le schéma réel.
