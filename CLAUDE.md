...
**Prochaine étape :** **vérifier `PUBLIC_APP_URL` côté Vercel** — `hub.jonlabs.ch` ne résout plus (NXDOMAIN) et c'est le fallback en dur de `notifications.ts` et du digest client, qui part le lundi 06:00 UTC avec des liens dedans. Ensuite, au choix : **AGT-001** (API agent v1 — approuver n'exécute toujours rien, 18 propositions en attente), **TEL-002** (rien n'envoie le rapport hebdo, il faut aller le lire sur `/reports`) ou le **portage de `/positions` sur le canon** (ex-onglet « Mots-clés », débloque FIND-007). **FIND-007** reste `BLOCKED` sur GSC-005. Détail : `docs/HANDOFF.md`.
