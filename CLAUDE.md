...
>
> ✅ **Le cockpit est en production depuis le 2026-07-27 ~19:40 UTC.** Depuis le **2026-07-30**, `main` = `origin/main` = prod = **`d222c39`** (`/api/whoami` → `version: d222c39`, 9 projets), et **4 crons** sont déclarés (`tick` horaire, `gmb-publish`, `linkedin-publish`, `gmb-weekly-digest`) — **`gmb-reviews` a été retiré** par GMB-002, la collecte d'avis passe par la file. Ce déploiement porte aussi le ciblage GMB par établissement (`ffba18e`) et la déduplication de `gmb_reviews` (`d222c39`). Les 9 cadences hebdo sont **en pause en base** — reprise projet par projet (`scripts/pauses.ts`). L'ancien risque `npm run db:push` (29 tables déclarées pour 61 en base) est **levé** : `main` déclare enfin le schéma réel.
...
**Avis GMB (E08/GMB-002 lot 1, 2026-07-28) :** la synchro est un **job de la file**
...
filtre `!reply && <30 j` mentait par omission.

**Avis GMB (E08/GMB-002 lot 2, 2026-07-28) — GMB-002 est CLOS :** `detect:review_pending`
produit `review_pending_sla` et `negative_review`. **Zéro DDL** (les deux types étaient déjà dans
`FINDING_TYPES`, `'review'` dans `FINDING_ENTITY_TYPES`). Au catalogue **quotidien**, arête
**obligatoire** depuis `collect:gmb_reviews`, provider `none`. ⭐ **Les deux types COEXISTENT**
sur un même avis : §10.4 leur donne deux gestes (SLA → skill `gmb-review-responder` ; négatif →
escalade **humaine**, aucun skill), et l'absorption rendrait « avis négatif EN RETARD »
indiscernable de « avis négatif frais ». Corollaire : `negative_review` vise un avis 1–3★ **non
traité**, faute de quoi rien ne pourrait jamais le résoudre. ⭐ **Le glissement de fenêtre ne
peut PAS auto-résoudre** : la borne des 180 j vit dans la closure **et** dans le scope — sans
cette symétrie, 332 avis toujours sans réponse seraient auto-résolus. ⭐ **Le plafond se répartit
par FICHE** (tour d'équité, une première dans le parc) : un plafond global aurait donné les 30
places à Eaux Vives et Jonction (74 % de l'arriéré) et **tronqué le 2★ de Sion**. ⭐ **Le scope
vient de la santé de synchro, statut ET fraîcheur** — le collecteur écrit `last_sync_at` aussi en
cas d'échec, donc la date seule dirait « synchronisée » d'une fiche en panne. ⚠️ **`create_time`
est en ISO** quand tout le reste est au format DB : le SQL ne pré-filtre que sur une **date nue**
et tout l'âge passe par `parseReviewCreateTime`. ⚠️ **Aucune PII dans les preuves** —
`finding_events.payload_json` est append-only. ⚠️ **Premier run réel : 17 findings sur
`barberconcept`** (13 SLA + 4 négatifs, 3 notifiables §14.3) — Eaux Vives 6 · Jonction 5 ·
Cornavin 4 · Sion 2 · **Lausanne 0**, la contre-épreuve. ⚠️ **`outOfWindow` vaut
structurellement 0** : la borne de lecture est le max des deux fenêtres (365 j), ~1 700 avis ne
sont pas lus du tout. ⚠️ **Une preuve doit tourner sur un projet vierge sur TROIS points**
(aucune fiche, aucun finding d'avis, aucun finding en veille) : `reconcileDetectionRun` et
`expireSnoozes` travaillent à l'échelle du projet.
...
**Chantier transverse :** migration Turso → Neon **TERMINÉE et déployée** (cutover 2026-07-26 14:35 UTC, `main` = `e5efc83`). Turso est figé et n'est plus lu. Les deux gestes d'infra restants (rotation du password, suppression de Turso) ne sont plus bloqués. ✅ **Le cockpit (E00) EST en prod depuis le 2026-07-27** (`main` = `7a8e04b`) : `/jobs`, `/inbox`, `/reports`, `/automations` et le cron `tick` tournent. ✅ **Le canary est TERMINÉ (2026-07-31) : les 9 cadences hebdo sont reprises**, et le run hebdo a réellement tourné sur les 9 projets — plus une seule pause active (`scripts/pauses.ts`).
**DB prod :** Neon `neondb`, schéma `seostats` (partagé avec `invoices` via schéma `core`). **61 tables** en base, et `main` les déclare toutes depuis le merge du cockpit. 9 projets.
...
**Deux correctifs nés du premier run réel (2026-07-31, `499d338` + `6cc2a22`) :** ⭐ **Un `fetch` sans plafond de temps ne peut être interrompu par personne.** Les boucles de collecte testent le signal du job (bail perdu, budget dépassé) **entre** deux appels seulement : un appel mort ne repasse par aucun test, la fonction Vercel est tuée à 300 s, et un `SIGKILL` ne repasse par aucun `finally` — la ligne reste `running` jusqu'au reaper, qui la rend à la file pour que la tentative suivante meure pareil (mesuré : `cardrank`, deux tentatives d'une heure chacune). Le plafond vit dans `resolveFetch` (`gsc-auth.ts`), donc couvre les **4** appels du module ; le dépassement devient un `GscApiError` **504** et jamais un `AbortError` nu — c'est ce qui le fait classer `retryable` au lieu de partir en dead-letter ; et le signal de l'appelant est **combiné**, pas remplacé. ⭐ **Un run ne peut plus se dire `success` sur un sous-ensemble de ses jobs** : un step n'est écrit qu'à l'issue **terminale** de son job, donc un job encore en file n'en a aucun, et son absence était indiscernable de son inexistence. `recomputeRunStatus` compte désormais les jobs `queued`/`running` du run. ⚠️ Ce n'était **pas** cosmétique : `classifyProjectReadiness` mappe `success` → `ready`, donc le rapport hebdo pouvait s'annoncer **`complete` alors qu'une détection n'avait pas tourné** — « absent ≠ zéro », transposé des sections du rapport aux steps d'un run.

> ✅ **Le mur du premier tick est FRANCHI (2026-07-31), et il a coûté deux défauts.** Les 9 cadences hebdo, suspendues au merge dans `automation_pauses` (portée `project_cadence`, journal append-only — **pas** `project_projections.payload.schedules`, une projection **recompilée** où la suspension serait effacée sans bruit), ont été reprises une par une (`scripts/pauses.ts --resume <slug> --reason "…" --execute`), et **le run hebdo a été FORCÉ le soir même** au lieu d'attendre le lundi : `scripts/schedule.ts --project=<slug> --now=2026-07-27T07:05:00Z --lookback-hours=1 --execute`. ⚠️ **La fenêtre de rattrapage resserrée à 1 h est ce qui évite de replanifier au passage le run QUOTIDIEN du même créneau** (07:00 local) — sans elle, chaque projet repart avec 14 jobs au lieu de 9.
>
> Mesures réelles, à substituer aux estimations d'avant : le catalogue hebdo porte **5 détecteurs (9 entrées)** depuis FIND-008, donc **81 jobs** pour 9 projets, pas 63. Un run d'un projet se draine en **UN tick** (~7 min) et non trois : le worker enchaîne les tours dans la même invocation, donc la profondeur 3 du graphe **ne coûte plus un tick par niveau** — c'est `MAX_JOBS_PER_TICK = 25` qui commande désormais. Le parc est passé de 36 à **171 findings ouverts** (jonlabs 79, barberconcept 22, lecureux 17) et de 4 à **18 propositions**.
>
> ⚠️ **`report.publish_deadline_minutes` a été porté à 300** (`system_settings`, échéance 14:00 locales au lieu de 10:00) **avant** la reprise : 81 jobs à 25 par tick concluent vers 12:00, et le défaut de 60 min aurait publié à 10:00 un rapport dont les détecteurs n'avaient pas encore tourné. Ce n'est pas une étiquette `partial` de plus — le contenu est construit à la publication.
>
> ⚠️ **Une pause de cadence n'empêche ni le drain ni REP-003** : le tick publie le rapport hebdo quand même, et celui du 2026-07-27 est un constat d'absence (`partial`, 9 projets `missing`) qui ne sera pas réécrit — **révisable** (`rep-003-publish.ts --revise <slot> --reason "…"`), et il le mérite maintenant que les runs de ce créneau existent et ont réussi. Depuis JOB-004, `propose:actions` **attend** la fin de la détection ; depuis JOB-006, il ne peut plus prendre le tick entier (tour d'équité).
