...
**Chantier transverse :** migration Turso → Neon **TERMINÉE et déployée** (cutover 2026-07-26 14:35 UTC, `main` = `e5efc83`). Turso est figé et n'est plus lu. Les deux gestes d'infra restants (rotation du password, suppression de Turso) ne sont plus bloqués. ✅ **Le cockpit (E00) EST en prod depuis le 2026-07-27** (`main` = `7a8e04b`) : `/jobs`, `/inbox`, `/reports`, `/automations` et le cron `tick` tournent. La mise en prod a été faite en **canary** — les 9 cadences hebdo sont en pause, reprise projet par projet, `barberconcept` en dernier.
...
**Prochaine étape :** **observer le premier tick en prod**, puis reprendre `lecureux` en canary (étape 5/5 de la mise en prod). Ensuite : le **portage de `/positions` sur le canon** (ex-onglet « Mots-clés », débloque FIND-007) ou **AGT-001** (API agent v1 — approuver n'exécute toujours rien). **FIND-007** reste `BLOCKED` sur GSC-005. Détail : `docs/HANDOFF.md`.
...
> ⚠️ **Le mur du premier tick a été désamorcé, pas supprimé — il attend chaque reprise de cadence.** La décision de Jonathan au merge (2026-07-27) a été de **suspendre** : les 9 cadences hebdo sont en pause `automation_pauses` (portée `project_cadence`, journal append-only, reprise auditable — **pas** `project_projections.payload.schedules`, une projection **recompilée** où la suspension serait effacée sans bruit), et chaque projet se reprend un par un via `scripts/pauses.ts --resume <slug> --reason "…" --execute`. Ce qui attend à la reprise : le catalogue hebdo porte **4 détecteurs** (7 entrées) et le plafond `maxCandidates` s'applique **par détecteur** — jusqu'à 50 opportunités **+** 50 baisses **+** 50 nouveautés par projet, soit **150 findings d'un coup sur `barberconcept`** (jamais détecté), et **63 jobs hebdo** si les 9 repartent ensemble pour `MAX_JOBS_PER_TICK = 25`. D'où l'ordre : `lecureux` d'abord (findings existants, collecte GSC fraîche), `barberconcept` en dernier. ⚠️ **Une pause de cadence n'empêche ni le drain ni REP-003** : le tick publie le rapport hebdo quand même, et le premier est un constat d'absence (`partial`, 9 projets `missing`) qui ne sera pas réécrit — **révisable** (`rep-003-publish.ts --revise <slot> --reason "…"`). Depuis JOB-004, `propose:actions` **attend** la fin de la détection au lieu de partir en parallèle ; depuis JOB-006, il ne peut plus prendre le tick entier (tour d'équité).
