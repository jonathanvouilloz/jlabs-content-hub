...
- `weekly_reports` (REP-003) : le rapport hebdo **publié**, 1 ligne par créneau (`period_slot` = créneau LOCAL `2026-07-27T09:00`, unique). **Aucun `project_id`** — première table du schéma dans ce cas, parce que le rapport est cross-projet. Porte `slot_at`/`due_at`/`published_at` (le SLO se **dérive**, jamais de colonne de verdict), `readiness_json` (attendus/prêts/dégradés/bloquants/écartés-pour-pause) et `payload_json` (le JSON REP-001 tel quel). **Jamais d'UPDATE** : republier un créneau est un no-op.
