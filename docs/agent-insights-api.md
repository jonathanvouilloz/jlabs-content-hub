...
Une collision d'identifiant entre les deux maps échoue fermée. Le credential doit déclarer :

- un `id` stable ;
- le scope `agent:insights:read` ;
- `projects`: allowlist explicite des slugs autorisés ;
- `expiresAt`: date d'expiration.

Au 24 septembre 2026, Barber Concept utilise le credential combiné
`hermes-barberconcept-2026-09-r2`, borné à `projects: ["barberconcept"]`, qui porte également les
quatre scopes d'avis. Le détail d'installation et de rotation se trouve dans
...
1. Définir `HERMES_MACHINE_CREDENTIALS_JSON` en Production dans Vercel en préservant les autres entrées utiles.
2. Définir le bearer Hermes dans son environnement sécurisé.
3. Redéployer : un changement de variable ne modifie pas un déploiement déjà construit.
4. Vérifier : appel valide sur un slug autorisé → `200` ; bearer absent → `401` ; slug hors allowlist → `403`.
5. Révoquer en retirant l'entrée de la variable dédiée ou en posant `revokedAt`.
