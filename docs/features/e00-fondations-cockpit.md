...
| **`src/routes/(app)/+page.svelte` · `projects/[slug]/+page.svelte`** *(DASH-003 lot 2 ch.1)* | `paused` ajouté à **`STATE_META`** et **`STATE_ORDER`** dans les deux écrans. ⚠️ Le repli `?? STATE_META.unknown` n'aurait pas planté : il aurait affiché « État inconnu » **en violet** sur un projet dont la cause du silence est connue, écrite et datée — le badge n'aurait pas cassé, il aurait **menti**. Bandeau bleu (jamais rouge ni ambre : c'est une décision), rendu **aussi sur une pause partielle** qui ne porte pas le badge — c'est justement le cas où la cause du silence est la plus dure à retrouver. **`project-cockpit.ts` n'est PAS touché** : la carte vient de `loadHomeCockpit` par slug, donc `pause` arrive gratuitement et l'invariant anti-divergence tient **par construction**. |
| **`scripts/dash-003-pause-health-proof.ts`** | **Preuve sur Neon (26 vérifs), ZÉRO réseau.** Le sentinelle est rendu **sain** (intégration GSC fraîche + 2 jobs détecteurs `succeeded`) — sans quoi « ce n'est pas `ok` » serait vrai **avant** la pause et §C ne prouverait rien. Trois contre-épreuves : la **reprise** rend une carte identique champ par champ (aucun état dérivé n'a été stocké) · une pause `monthly` rend une carte **identique** · une échéance dépassée lève la pause avec `count(automation_pauses)` **inchangé**. Plus l'ordre des règles contre la vraie base (`revoked` + gel ⇒ `broken`) et l'égalité accueil/cockpit **pause active**. ⚠️ **Les décisions sont horodatées en instants strictement croissants** : `created_at` est à la **seconde** et le départage `id DESC` est reproductible **mais pas juste** — la 1ʳᵉ version posait pause et reprise dans la même seconde et échouait une fois sur deux, en accusant le code d'un défaut inexistant. Slug emprunté à `core.entities`, nettoyage **par id, jamais par slug**. |
