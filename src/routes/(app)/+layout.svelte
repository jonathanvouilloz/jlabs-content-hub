<script lang="ts">
	...
	function projectNav(slug: string) {
		return [
			...
			{ href: `/projects/${slug}/gmb-profile`, label: 'Fiche Google', icon: Building2, exact: false, linkedin: false },
	...
	>
		...
		<nav class="flex-1 overflow-y-auto {collapsed ? 'px-1.5' : 'px-2.5'} py-3">
			...
			<a
				href="/seo"
				class="mb-0.5 flex items-center gap-2.5 rounded-md {collapsed ? 'justify-center px-0 py-2' : 'px-2.5 py-1.5'} text-sm font-medium transition-colors
					{isActive('/seo') ? 'bg-surface-100 text-surface-900' : 'text-surface-500 hover:bg-surface-50 hover:text-surface-900'}"
				title={collapsed ? 'SEO' : undefined}
			>
			...
			<a
				href="/automations"
				class="mb-0.5 flex items-center gap-2.5 rounded-md {collapsed ? 'justify-center px-0 py-2' : 'px-2.5 py-1.5'} text-sm font-medium transition-colors
					{isActive('/automations') ? 'bg-surface-100 text-surface-900' : 'text-surface-500 hover:bg-surface-50 hover:text-surface-900'}"
				title={collapsed ? 'Automatisations' : undefined}
			>
				<CalendarClock size={16} class="flex-shrink-0" />
				{#if !collapsed}<span>Automatisations</span>{/if}
			</a>
